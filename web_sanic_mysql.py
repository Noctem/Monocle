...
async def spawn_points(request):
    async with app.pool.acquire() as conn:
        async with conn.cursor(DictCursor) as cur:
            results = await cur.fetchall('SELECT spawn_id, despawn_time, lat, lon, duration FROM spawnpoints')
    return json(results)
...
async def get_pokestops(request):
    async with app.pool.acquire() as conn:
        async with conn.cursor(DictCursor) as cur:
            results = await cur.fetchall('SELECT external_id, lat, lon FROM pokestops')
    return json(results)
